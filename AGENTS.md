...
- Dashboard work: `docker compose -f stack/docker-compose.dev.yml --profile dashboard up -d` adds Prometheus on 9091 and Grafana on 3001 (anonymous admin, no login). The dashboard is provisioned read-only from `stack/grafana/dashboards/pi-otlp-dashboard.json`, so edit the JSON and restart Grafana rather than editing in the UI.
- **`increase()` is the wrong function for range totals here, and reads 0.** Every counter is labelled with `session_id`, so each session gets its own series that is born carrying that session's whole contribution and never changes. Prometheus does not count a series' first sample as a rise from zero, so a flat series has no increase. Range totals (`[$__range]`) therefore use `max_over_time`, which sums each session's final value; the activity-over-time panels (`[$__rate_interval]`) keep `increase`/`rate`, which is correct there and works whenever a counter climbs across scrapes. Details and the measured comparison: [plan 01](docs/plans/01-dashboard-fixes.md).
- The dev replay fires all nine hook events within ~2 seconds, inside a single 15s scrape, so the `[$__rate_interval]` panels read zero against it. That is the replay, not the dashboard — pace events >15s apart to see those panels populate.
...
- **Teardown is bounded by the bridge, not the SDK — don't "simplify" it back to a bare `await forceFlush()`.** When the reader's export timeout fires it abandons the in-flight request without cancelling it, then still awaits `exporter.forceFlush()`, so against an unreachable collector `meterProvider.forceFlush()` never settles and no SDK-side timeout helps. `settleWithin` caps the wait and the unconditional `process.exit(0)` reclaims the socket, which otherwise holds the event loop for the full TCP SYN-retry window. Measured: >120s → 3.1s per hook. Also note `exportTimeoutMillis` is clamped to `exportIntervalMs` because the reader throws if it exceeds it and `main()` would swallow that. [plan 04](docs/plans/04-sync-hook-exporter-timeout.md).
- `prompt.length.bucket` (not a raw character count) is on `pi.prompt.count`. Both emitters import `promptLengthBucket` from `pi/src/attributes.ts` — a bucketing difference between them would split one Prometheus series in two, so keep it shared.
